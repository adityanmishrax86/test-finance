  ...
  passport.authenticate("local", (err, user) => {
    ...
    if (!user) {
      return res.status(401).send({
        status: false,
        message:
          "Either Provided Username or password is incorrect. Please Check the credentials.",
      });
    }
    if (!req.session.jwt) {
  ...
  try {
    ...
    if (user !== null) {
      return res.status(200).send({
        status: false,
        message:
          "User is already registered with us. Please Login with the given Email address.",
      });
    }
